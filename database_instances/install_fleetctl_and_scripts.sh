...
control_ip=$(gcloud compute instances list --project=$project | grep -v grep | grep -m 1 pro-database-control | awk {'print $5'})
...
FLEET_RELEASE=$(gcloud compute --project=$project ssh --zone=$zone "core@pro-database-control" --command "fleetctl version | cut -d ' ' -f 3- | tr -d '\r' ")
